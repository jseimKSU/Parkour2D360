...
namespace Parkour2D360.Collisions
{
    public static class CollisionHelper
    {
        ...
        public static bool IsCharacterStandingOnAnEntity(
            BoundingRectangle character,
            BoundingRectangle entity
        )
        {
            if (
        ...
        {
            return (
                    (topOfEntity > bottomOfCharacter)
                        ? (topOfEntity - bottomOfCharacter)
                        : COLLIDING_BUFFER + 1
                ) > COLLIDING_BUFFER;
        }

        private static bool CharacterIsTooFarToTheLeft(
            float rightSideOfCharacter,
            float leftSideOfEntity
        )
        {
            return (leftSideOfEntity - rightSideOfCharacter) > COLLIDING_BUFFER;
        }

        private static bool CharacterIsTooFarToTheRight(
            float leftSideOfCharacter,
            float rightSideOfEntity
        )
        {
        ...
        public static float GetYFromXPosition(
            Vector2 platformStartingVertex,
            float otherHitboxXPosition,
            float vertexAngle
        )
        {
            return (float)(
                (platformStartingVertex.X - otherHitboxXPosition) * Math.Tan(vertexAngle)
            );
        }
    }
}
